...
class ViolaJonesViewController: UIViewController
{
    ...
    {
        timer = Timer.scheduledTimer(timeInterval: TimeInterval(timerTimeInterval), target: self, selector: #selector(onGenerateViolaJonesImages), userInfo: nil, repeats: true)
    }
    ...
    //----------------------------------------------------------------------------
    @objc func onGenerateViolaJonesImages()
    {
        AF.request(
            "http://127.0.0.1:5000/generate-viola-jones-images",
            method: .get,
